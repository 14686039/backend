...
//拦截器签名，拦截的目标类型是StatementHandler，拦截的方法是名称为prepare参数为Connection类型的方法
	...
	public Object intercept(Invocation ivk) throws Throwable {
		if (ivk.getTarget() instanceof RoutingStatementHandler) {
			...
			if (mappedStatement.getId().matches(pageSqlId)) { // 拦截需要分页的SQL
				System.out.println("===========拦截到了需要分页的SQL:"+mappedStatement.getId()+"===========");
				...
				if (parameterObject == null) {
					...
					BoundSql countBS = new BoundSql(
							...
							boundSql.getParameterMappings(), parameterObject);//创建了查询总数的BoundSql对象
					setParameters(countStmt, mappedStatement, countBS, parameterObject);
					ResultSet rs = countStmt.executeQuery();//执行SQL语句的结果集
					int count = 0;
					if (rs.next()) {
						count = rs.getInt(1);
					}
					...
					if (parameterObject instanceof Page) { // 参数就是Page实体
					...
					} else if (parameterObject instanceof Map) { //参数是Map实体
						...
						if (pageField != null) {
							...
							if (page == null)
								page = new Page();
							page.setTotalCount(count);
							ReflectHelper.setValueByFieldName(parameterObject, "page", page); // 通过反射，对实体对象设置分页对象（page）
						} else {
