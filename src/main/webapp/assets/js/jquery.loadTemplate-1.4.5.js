    ...
    function loadAndPrepareTemplate(template, selection, data, settings) {
        ...
        $.ajax({
            ...
            success: function (templateContent) {
                ...
                $templateContainer.html(templateContent);
//              if("string"=== typeof templateContent)
//              	$templateContainer.html(templateContent);
//              else
//              	$templateContainer.html(templateContent.documentElement.outerHTML);
    ...
    function bindData(template, data, settings) {
        ...
        processElements("data-pagerStatus", template, data, settings, function ($elem, value) {
            $elem.attr("pagerStatus", applyFormatters($elem, value, "pagerStatus", settings));
        });
        
        processElements("data-name", template, data, settings, function ($elem, value) {
            $elem.attr("name", applyFormatters($elem, value, "name", settings));
        });
        
        processElements("data-menucode", template, data, settings, function ($elem, value) {
            $elem.attr("menucode", applyFormatters($elem, value, "menucode", settings));
        });
        processElements("data-url", template, data, settings, function ($elem, value) {
            $elem.attr("url", applyFormatters($elem, value, "url", settings));
        });
        ...
        processElements("data-checked", template, data, settings, function ($elem, value) {
        	if(value=="1"){
        		$elem.attr("checked", applyFormatters($elem, value, "checked", settings));
        	}
        });
         ...
         processElements("data-options", template, data, settings, function ($elem, value) {
            $(value).each(function () {
                var $option = $("<option/>")
                if(this.indexOf(":selected")>=0){
                	$option.attr('value', this.replace(":selected","")).attr('selected','selected').text(this.replace(":selected","")).appendTo($elem);
                }else if(this.indexOf(":")>0&&this.indexOf(":selected")<0){
                	$option.attr('value',this.substring(0,this.indexOf(":"))).text(this.substring(this.indexOf(":")+1)).appendTo($elem);
                }else{
